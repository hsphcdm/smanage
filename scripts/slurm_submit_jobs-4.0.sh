...
if [[ -z $BIN ]]; then
BIN=/n/seage_lab/model_versions/release/transm-v4.0.4
fi
if [[ -z $INFILES ]]; then
INFILES=/n/seage_lab/esurface/BWA_Calibration/simulations/INFILES
fi
if [[ -z $NEXT_RUN_ID ]]; then
NEXT_RUN_ID=0
fi
if [[ -z $BATCH_PREFIX ]]; then
BATCH_PREFIX=Bots_ART_v4_batch_
fi
...
SBATCH_SCRIPT=$top/run_trans_model_batches-4.0.sh
fi
